...
public class EmployeeRepositoryImpl implements EmployeeRepository {
	...
	public int insertEmployee(Employee employee) throws HRSSystemException{
		...
		try {
			PreparedStatement pStmt = conn.prepareStatement("insert into EMPLOYEE (ID, FIRSTNAME, LASTNAME, PROF_LEVEL) VALUES (EMPLOYEE_SEQ.NEXTVAL, ?,?,?)");
			...
			conn.commit();
			if (rows == 1) {
				pStmt = conn.prepareStatement("SELECT EMPLOYEE_SEQ.CURRVAL FROM DUAL");
